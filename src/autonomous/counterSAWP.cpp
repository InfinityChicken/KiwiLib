...
void counterSAWP() {
    ...
    chassis.sendVoltage(6000, 500); //prev 400, 350
    ...
    //go to long
    chassis.turnToPoint(-41, -60, 1000, {.minSpeed = 30, .earlyExitRange = 4});
    chassis.moveToPoint(-41, -50, 1000, {.minSpeed = 60, .earlyExitRange = 4});
    chassis.turnToHeading(180, 1000);
    chassis.distanceReset('R', 'F');
    chassis.moveToPoint(-47, -25, 1000, {.forwards = false, .minSpeed = 60, .earlyExitRange = 4});
    intakeState = 3;
    trapdoorState = 1;
    chassis.sendVoltage(-12000 * 0.3, 1000); //prev voltage 50%, prev time 800, 700
    chassis.setPose(-1, -1, chassis.getPose().theta);
    chassis.distanceReset('R', 'F');

    //go to ml
    trapdoorState = 0;
    chassis.moveToPoint(-47, -48, 1000, {.minSpeed = 60, .earlyExitRange = 4});
    chassis.turnToHeading(180, 1000);
    chassis.moveDistance(4, 1000, {.minSpeed = 127 * 0.5});
    chassis.sendVoltage(6000, 750); //prev 400
    chassis.turnToHeading(180, 1000);
    chassis.distanceReset('R', 'F');
    ...
    chassis.distanceReset('R', 'F');
    chassis.moveToPose(-10, -13 , 220, 2000, {.forwards = false, .lead = 0.5, .minSpeed = 80});
